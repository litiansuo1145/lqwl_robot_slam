...
class DataSubscriber(Node):
    def __init__(self, name, port = '/dev/dock_32car', baudrate = 115200, ticks_per_meter = 180, base_width = 0.17):
        super().__init__(name)
    ...
    def Car_Control(self):
        while rclpy.ok(): 
            ...
            if self.output_speedX > 0:
                if self.output_speedX > 6: 
                    self.output_speedX = 6
                elif self.output_speedX < 4:
                    self.output_speedX = 4
            ...
            if self.output_gyroz > 0:
                if self.output_gyroz > 2:
                    self.output_gyroz = 2
                elif self.output_gyroz <0.1:
                    self.output_gyroz = 0.1
